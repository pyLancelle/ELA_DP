...
Usage:
    python -m src.connectors.garmin.garmin_ingest_auto --env dev
    python -m src.connectors.garmin.garmin_ingest_auto --env prd --dry-run
"""
...
FILE_PATTERNS = {
    r'.*_activities\.jsonl$': 'activities',
    r'.*_activity_details\.jsonl$': 'activity_details',
    r'.*_activity_splits\.jsonl$': 'activity_splits',
    r'.*_activity_hr_zones\.jsonl$': 'activity_hr_zones',
    r'.*_activity_weather\.jsonl$': 'activity_weather',
    r'.*_sleep\.jsonl$': 'sleep',
    r'.*_floors\.jsonl$': 'floors',
    r'.*_weight\.jsonl$': 'weight',
    r'.*_steps\.jsonl$': 'steps',
    r'.*_heart_rate\.jsonl$': 'heart_rate',
    r'.*_body_battery\.jsonl$': 'body_battery',
    r'.*_stress\.jsonl$': 'stress',
    r'.*_hrv\.jsonl$': 'hrv',
    r'.*_training_status\.jsonl$': 'training_status',
    r'.*_race_predictions\.jsonl$': 'race_predictions',
    r'.*_endurance_score\.jsonl$': 'endurance_score',
    r'.*_hill_score\.jsonl$': 'hill_score',
}
...
def detect_data_type(filename: str) -> str | None:
    ...
    Args:
        filename: Name of the file (e.g., "2025_10_28_garmin_activities.jsonl")

    Returns:
        Data type (e.g., "activities") or None if not recognized
    """
...
def scan_landing_folder(bucket_name: str, landing_path: str) -> Dict[str, List[str]]:
    ...
    Args:
        ...
        landing_path: Landing folder path (e.g., "garmin/landing")
...
def run_ingestion(data_type: str, env: str, dry_run: bool = False) -> Tuple[bool, str]:
    ...
    Args:
        data_type: Data type config name (e.g., "activities")
    ...
    cmd = [
        sys.executable,
        "-m", "src.connectors.garmin.garmin_ingest_v2",
        "--config", data_type,
        "--env", env
    ]
...
def run_auto_ingestion(env: str, dry_run: bool = False) -> int:
    ...
    """
    logging.info(f"\n{'='*80}")
    logging.info("GARMIN AUTO-INGESTION")
    logging.info(f"{'='*80}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Garmin Auto-Ingestion: Automatically detect and ingest all data types',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Ingest all Garmin data types in dev
  python -m src.connectors.garmin.garmin_ingest_auto --env dev
  ...
  python -m src.connectors.garmin.garmin_ingest_auto --env dev --log-level DEBUG
        """
    )
