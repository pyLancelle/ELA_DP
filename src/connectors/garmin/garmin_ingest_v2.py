#!/usr/bin/env python3
"""
Generic Garmin Data Ingestion Engine - Configuration-Driven Approach
=====================================================================

This script provides a declarative, configuration-driven ingestion pipeline
for Garmin data with support for hybrid parsing (typed columns + raw JSON).
...
Usage:
    # Basic ingestion
    python -m src.connectors.garmin.garmin_ingest_v2 --config activities --env dev

    # With options
    python -m src.connectors.garmin.garmin_ingest_v2 \\
        --config activities \\
        --env prd \\
        --log-level DEBUG \\
        --dry-run
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='Generic Garmin data ingestion with YAML configuration',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Ingest activities to dev
  python -m src.connectors.garmin.garmin_ingest_v2 --config activities --env dev

  # Ingest to prod with debug logging
  python -m src.connectors.garmin.garmin_ingest_v2 --config activities --env prd --log-level DEBUG

  # Dry run (validate only)
  python -m src.connectors.garmin.garmin_ingest_v2 --config activities --env dev --dry-run

  # Process specific file
  python -m src.connectors.garmin.garmin_ingest_v2 --config activities --env dev \\
      --file gs://ela-dp-dev/garmin/landing/2024_10_29_activities.jsonl
        """
    )
    ...
    parser.add_argument(
        '--config',
        required=True,
        help='Configuration name (e.g., activities, sleep)'
    )
    ...
    try:
        # Run ingestion
        ingestor = GarminIngestor(config_path, args.env, args.dry_run)
