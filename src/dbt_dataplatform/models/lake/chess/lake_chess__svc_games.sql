...
WITH games_data_with_rank AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY JSON_EXTRACT_SCALAR(raw_data, '$.uuid')
      ORDER BY dp_inserted_at DESC
    ) AS row_rank

  FROM {{ source('chess', 'lake_chess__stg_chess_raw') }}
  WHERE data_type = 'games'
    AND JSON_EXTRACT_SCALAR(raw_data, '$.uuid') IS NOT NULL
