...
-- Pure Lake model for Chess.com tournaments data
-- Stores raw JSON data with basic metadata and deduplication only
-- All field extraction logic will be moved to Hub layer

WITH tournaments_data_with_rank AS (
  SELECT
    -- Unique identifier for deduplication
    JSON_EXTRACT_SCALAR(raw_data, '$.@id') AS tournament_id,
    ...
    ROW_NUMBER() OVER (
      PARTITION BY JSON_EXTRACT_SCALAR(raw_data, '$.@id')
      ORDER BY dp_inserted_at DESC
    ) AS row_rank

  FROM {{ source('chess', 'lake_chess__stg_chess_raw') }}
  WHERE data_type = 'tournaments'
    AND JSON_EXTRACT_SCALAR(raw_data, '$.@id') IS NOT NULL
...
FROM tournaments_data_with_rank
WHERE row_rank = 1