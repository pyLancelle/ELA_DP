...
-- Hub model for Garmin running activities segments data
...
-- Note: This is an alternative view to timeseries, potentially for segment-based analysis
...
WITH timeseries_extract AS (
  ...
  WHERE JSON_VALUE(raw_data, '$.activityType.typeKey') = 'running'
    AND JSON_QUERY(raw_data, '$.detailed_data.activityDetailMetrics') IS NOT NULL
  {% if is_incremental() %}
