...
class TestTodoistExtraction(unittest.TestCase):
    ...
    def test_fetch_projects(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: [{"id": 1, "name": "Test Project"}])
    ...
    def test_fetch_sections(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: [{"id": 2, "name": "Section"}])
    ...
    def test_fetch_tasks(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: [{"id": 3, "content": "Task"}])
    ...
    def test_fetch_activity_logs(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"events": [{"id": 4, "event": "added"}]})
    ...
    def test_fetch_tasks_malformed(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200, json=lambda: {"unexpected": "data"})
